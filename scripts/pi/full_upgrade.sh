...
echo "[1/${STEPS}] update registry"
sudo apt update

echo "[2/${STEPS}] list upgrades"
sudo apt list --upgradeable


echo "[3/${STEPS}] remove docker config before update process at SYS_DOCKER_CONFIG_PATH: ${SYS_DOCKER_CONFIG_PATH}"
sudo rm sudo rm $SYS_DOCKER_CONFIG_PATH


echo "[4/${STEPS}] full upgrade apt"
sudo apt full-upgrade -y


echo "[5/${STEPS}] now clean up"
